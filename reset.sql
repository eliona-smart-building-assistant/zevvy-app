...
DROP SCHEMA IF EXISTS zevvy CASCADE;

DELETE FROM heap
WHERE asset_id IN (
	SELECT asset_id
	FROM asset
	WHERE asset_type LIKE E'zevvy\\_%'
);
...
WHERE widget_id IN (
	...
	WHERE public.dashboard.name LIKE 'Zevvy%'
);
...
WHERE dashboard_id IN (
	...
	WHERE name LIKE 'Zevvy%'
);

DELETE FROM public.dashboard
WHERE name LIKE 'Zevvy%';

-- DELETE FROM eliona_app WHERE app_name = 'zevvy';
-- DELETE FROM eliona_store WHERE app_name = 'zevvy';
